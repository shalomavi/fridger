// GLSL sources for GhostFibers. Kept as plain strings, isolated from the
// component so the WebGL setup code isn't buried under shader text.
...
void main() {
  ...
  vec3 outputColor;
  if (uLightMode > 0.5) {
    // uBrightness/uGlowIntensity default to 2.0/1.6 — normalize against
    // those so the light branch reads the same "1.0 = as designed" scale
    // the dark branch's own tonemap uses, rather than a second unrelated
    // 0-1 range callers would have to learn separately.
    float inkStrength = clamp(uBrightness / 2.0, 0.0, 2.0);
