...
export function ScrollToTopButton({ label }: { label: string }) {
  ...
  return (
    <button
      ...
      aria-label={label}
      className={`fixed bottom-6 end-6 z-30 flex h-11 w-11 items-center justify-center rounded-full bg-primary text-white transition-transform duration-300 active:scale-95 ${buttonShadow}`}
    >
      <ArrowUpIcon />
    </button>
  )
